...
def read_dictionary():
    """
    This function will read the dictionary file and store the word into a dict.
    """
    ...
    with open(FILE, 'r') as f:
        for line in f:
            line = line.strip()
            for i in range(len(ALPHABET)):
                if line.startswith(ALPHABET[i]):
                    DICT[ALPHABET[i]].append(line)
...
def find_anagrams_helper(s, ans_len, order, anagrams):
    ...
    if len(order) == ans_len:
        for num in order:
            current_s += s[num]
        if current_s in DICT[current_s[0]] and current_s not in anagrams:
