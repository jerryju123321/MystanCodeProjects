...
def get_best_pixel(pixels):
    ...
    for pixel in pixels:
        if get_pixel_dist(best_pixel, avg[0], avg[1], avg[2]) > get_pixel_dist(pixel, avg[0], avg[1], avg[2]):
            best_pixel = pixel
...
def solve(images):
    ...
    for i in range(width):
        for j in range(height):
            pixels = []
            result_p = result.get_pixel(i, j)

            # get the pixel from each image in images to compare
            for image in images:
                pixels.append(image.get_pixel(i, j))

            best_p = get_best_pixel(pixels)  # the pixel which has the closest color to the average
